...
# def overlay_bbox(img, bbox):
#     img = img.copy()
#     for bb in bbox:
#         x_min_, y_min_, x_max_, y_max_ = bb
#         x_min_, y_min_, x_max_, y_max_ = int(x_min_),int( y_min_), int(x_max_), int(y_max_)
#         img[y_min_:y_max_, x_min_:x_min_+1,:] = 0
#         img[y_min_:y_min_+1, x_min_:x_max_,:] = 0
#         img[y_min_:y_max_, x_max_:x_max_+1,:] = 0
#         img[y_max_:y_max_+1, x_min_:x_max_,:] = 0
#     return img
...
class papSmearData:
    ...
    # def next_batch(self):
    #     batch = {'gt_boxes': [], 'gt_classes': [], 'dontcare': [], 'origin_im': []}
    #     img_batch = []
    #     i = 0
    #     while i < self.batch_size:   
    #         try:
    #             images, gt_boxes, classes, dontcare, origin_im = self.gen.__next__()
    ...
    #             indexes = list(range(self.img_num)) 
    #             if self._shuffle:
    #                 random.shuffle(indexes)
    #             self.img_shape = self._img_shape    
    #             self.gen = self.pool.imap(_get_next,  
    #                  ( (self.img_list[i], self.mat_list[i], self.img_list_[i], self.mat_list_[i],
    #                     self.resize_ratio,self.img_shape, self.testing) for i in indexes),  
    #                     chunksize=self.batch_size)    
    #             self._epoch += 1
    #             print(('epoch {} start...'.format(self._epoch)))
    #     # if self._epoch >= 2:
    #     #     for this_batch in img_batch:
    #     #         print(this_batch.shape)
    #     img_array = np.stack(img_batch, 0)
