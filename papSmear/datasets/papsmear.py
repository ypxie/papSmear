...
class papSmearData:
    def __init__(self, data_dir, batch_size=8, processes=8, img_shape=(256, 256),
        ...
        self.img_list  = [imread(img_path) for img_path in self.img_list_]
        self.mat_list  = [load_mat(mat_path, contourname_list=['Contours']) for mat_path in self.mat_list_]
    ...
    def next_batch(self):
        ...
        # ### Error when using self.pool multiprocessing
        ...
        # self.start += this_num
        # self.batch_count += 1
        # for ind in this_batch_indices:
        #     inputs = (self.img_list[ind], self.mat_list[ind], self.img_list_[ind], self.mat_list_[ind],
        #               self.resize_ratio,self.img_shape, self.testing)
        #     images, gt_boxes, classes, dontcare, origin_im = _get_next(inputs)
        #     if len(gt_boxes) > 0:
        #         # add to batch
        #         batch['images'].append(images)
        #         batch['gt_boxes'].append(gt_boxes)
