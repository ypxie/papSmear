...
from papSmear.darknet import BreastNet as Darknet19
...
data_root = os.path.join(home, 'DataSet','yoloSeg', 'train','isbi14')
save_root = os.path.join(data_root,'rectangle')
mkdirs(save_root)
seg_inchan = 35
model_name = 'isbi14_yolo_{}'.format(seg_inchan)
...
def set_args():
    ...
    parser.add_argument('--load_from_epoch', type=int, default= 7600, help='load from epoch')

    parser.add_argument('--display_freq',    type=int, default= 1000, help='plot the results every {} batches')
    parser.add_argument('--save_freq',       type=int, default= 200,  help='how frequent to save the model')
...
if  __name__ == '__main__':
    ...
    # Dataloader setting
    dataloader = Dataset(data_root, args.batch_size, img_shape = (256,256), resize_ratio=[0.3, 0.5, 0.6, 0.8])
    #dataloader.overlayImgs(save_root)
    # Set Darknet
    net = Darknet19(cfg, seg_inchan=seg_inchan)
